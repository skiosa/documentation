...
Ziel des ersten Sprintes war es die ersten Schritte im PI \enquote{Initial Infrastructure} (siehe Abschnitt \ref{initial_infra}) zu gehen.
Hierbei ging es darum die Arbeitsgrundlage für zukünftige Sprints zu legen.
Konkret war es also das Ziel zum einen die Grundlegende Infra aufzusetzten (Server und Domain zu kaufen und aufzusetzten), sowie die eingesetzten Collaborationstools, wie Jira, Confluence, Github und Figma vorzukonfigurieren.
Zum anderen aber auch die Grundarchitektur zu klären und erste Mockups in Figma zu erstellen.

\subsection{Probleme und Verbesserungen}
Im Retro des ersten Sprintes wurde das \enquote{Start, Stop, Continue} verfahren gewählt.
...
Als Konsequenz wurde hierraus gezogen, dass zusätzlich zu den Weeklys nun ein Bi-Weekly organisiert werden sollte und Sprint Kapazitäten vor den jeweiligen Sprints geschätzt werden sollten (Die Auswirkung dieser Änderung ist in Abschnitt \ref{orga_changes} nachzulesen).
...
Ziel des zweiten Sprintes war es das verfehlte Sprint Ziel des ersten Sprintes nachzuholen und somit das erste \ac{PI} fertig zu stellen. Zusätzlich sollten nun erste Backend Komponenten für das zweite \ac{PI} \enquote{Article Views and Base Homepage} erstellt werden.
Hier war es also nun das Ziel die Service Boilerplates und die darum stehende \ac{CI/CD} Infrastruktur yu erstellen, als auch mit diesen Boilerplates Endpoints für Artikel View Pages und die Homepage zu erstellen.
Auf Seite der Mockups war es nun das Ziel diese für die folgenden \acp{PI} zu erstellen. 

\subsection{Probleme und Verbesserungen}
In der Retro des zweiten Sprintes wurde auf Basis einer Schatzsuche mit Piraten, gefählichen Gewässern und Scätzen die Teamarbeit analysiert.
Hierbei kam zum Vorschein, dass häufig Tickets nicht allgemein verständlich beschrieben waren, zu wenig für die Abgabe Dokumentation bisher gemacht wurde und zu viel Arbeit in letzter Minute vor Sprintende erledigt wurde.
An positiver Seite wurde aber erläutert wie hier das Sprintziel wesentlich besser erreicht wurde und es viel Fortschritt und weniger Disskussionsbedarf gab.
Als Fazit wurden nun Deadlines für Stories erstellt und bei zukünftigen Estimation meetings sollten nun Statt vom Product Owner, Stories von Service Leads vorgestellt werden. Mit letzterem kann so sichergestellt werden, dass Stories erst bearbeitet werden können, sobald zumindest ein Service Lead dessen Inhalt versteht.
...
\section{Sprint 3}
Ziel des dirtten Sprintes war es die Frontend Komponenten des \ac{PI} \enquote{Article Views and Base Homepage} fertig zu erstellen und die Backend Komponenten des nächsten \ac{PI} \enquote{User Interaction} vorzubereiten.
Hierbei ging es darum im Frontend die Feed Overview Page, die Article View Page sowie die Homepage zu erstellen.
Zusätzlich war es nun das Ziel die restlichen Mockups für die letzten (im Zuge dieses Projektes machbaren \acp{PI}) zu erstellen und alle vergangenen Geschehnissen des Projektes zu dokumentieren.

\subsection{Probleme und Verbesserungen}
In der Retro des Dritten Sprintes wurde ein neues online Retro Tool probeweise genutzt. Da dieses Meeting online stattfand erlaubte das Tool trozt der Fehlenden Gemeinschaft einen effektiven Austausch.
Beim Ablauf ist aufgefallen, dass in diesem Sprint sehr erfolgreich viele Stories bearbeitet werden konnten und es zwischen Teammitgliedern weniger Reibungen gab. Nicht desto trotz wurde hervorgehoben, dass Reviews noch nicht gründlich genug waren. Als Lösungsvorschlag wurde hier erhoben, dass mehr Zeit für diese eingeräumt werden sollte.
...
\subsection{Sprint 4}
Ziel des letzten Sprints war es das \ac{PI} \enquote{User Interaction} fertig zustellen und die Abgabedokumentation für die Abgabe zurzubereiten.
Im Zuge des \acp{PI} bedeutetet dies das ertellen von Likes, Bookmarks und der Feed Adding Page.
...
\subsection{Probleme und Verbesserungen}
In der Retro des letzten Sprintes wurde erneut das Tool aus dem dritten Sprint verwendet.
Hierbei ist aufgefallen, dass nun Reviews besser gehandhabt wurden, deadlines im vergleich zu den restlichen Sprint besonders gut eingehalten worden sind und das Sprint Ziel erneut gut eingehalten worden ist.
Neue Action Items sind im laufe dieser Retro nicht entstanden, da keine weiteren Sprints vorgesehen sind.
Nicht desto trotz wurde ein generelles Fazit zum Projekt gezogen, welches in Kapitel \ref{proj_ergebnisse} nachzulesen ist.
